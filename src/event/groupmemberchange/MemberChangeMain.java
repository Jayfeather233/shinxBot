...
public class MemberChangeMain implements EventProcessable {
    ...
    public void process(JSONObject J) {
        ...
        if (J.getString("notice_type").equals("group_decrease")) Main.setNextSender("group",0,J.getLong("group_id"),name + "离开了我们……");
        if (J.getString("notice_type").equals("group_increase")) Main.setNextSender("group",0,J.getLong("group_id"),"欢迎 " + name + " 的加入");
    }
